...
tidy(yr_fit, conf.int = TRUE) %>% mutate(OR = exp(estimate),
...
tidy(yr_fit3, conf.int = TRUE) %>% mutate(OR = exp(estimate),
                                              conf_low = exp(conf.low),
                                              conf_high = exp(conf.high))
...
full_model <- svyglm(
  bp_control_140_90 ~ svy_year2 +
    ...
    bp_med_n_class2 + bp_med_n_pills +htn_aware + htn_resistant_jnc7,
  design = cardio_svy,
  family = quasibinomial())
...
X3 <- model.matrix(~svy_year2 +
    ...
    bp_med_use + bp_med_n_class2 + bp_med_n_pills +htn_aware + htn_resistant_jnc7,
        data = data.frame(X2))
...
selected_model <- stepwise_model_selection(
  ...
  predictor_variables = c("svy_year2","demo_age_cat","demo_gender", "demo_race","cc_smoke","cc_bmi","cc_diabetes","cc_ckd","cc_cvd_any","bp_med_use" ,"bp_med_n_class", "bp_med_n_pills" ,"htn_aware" , "htn_resistant_jnc7"),
...
regTermTest(final_model, "bp_med_n_class")
regTermTest(final_model, "bp_med_use")

tidy(final_model, conf.int = TRUE) %>% mutate(OR = exp(estimate),
                                              conf_low = exp(conf.low),
                                              conf_high = exp(conf.high))
...
regTermTest(interaction_model, "svy_year2:demo_age_cat") ## significant!
...
pred1 = pred0
pred1[, c("svy_id", "cont_svy_yr", "demo_age_cat","bp_med_n_class2", "svy_year2")] = 0
pred1
...
tidy(pooled_results, conf.int=TRUE) %>% mutate(OR=estimate,
                                               conf_low = exp(conf.low),
                                               conf_high = exp(conf.high)) 
...
"bp_med_n_classTwo" = "2 or more antihypertensive med",
...
rows <- tibble::tribble(
  ...
  "BMI: <25", "ref",
  "No antihypertensive med", "ref")
attr(rows, "position") <- c(1, 5, 9, 14, 17, 24)


tables <- list()
tables[['Odds Ratio [95% CI]']]<- pooled_results
...
reg_tab2 <- modelsummary(
  ...
  stars = TRUE,
  coef_omit = c(1, 20, 23, 24), coef_rename = my_renames,
  gof_omit = ".*", ## drop num obs, num of imp etc.
  add_rows = rows,
  statistic=NULL,
  fmt =  fmt_sprintf("%.3f")) |> autofit()
...
reg_plot1 <- modelplot(pooled_results, 
                       exponentiate = TRUE, 
                       coef_omit = c(1, 20, 23, 24), 
